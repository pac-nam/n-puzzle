...
func search(ctx *s.SContext, cost, costMax, heuristicScore int, m map[string]interface{}) ([]Tnumber, bool) {
	if heuristicScore == 0 {
		return make([]Tnumber, 1), true
	} else if cost + heuristicScore > costMax {
		return nil, false
	}
	PuzzleString := t.PuzzleToString(ctx.Puzzle)
	m[PuzzleString] = nil
	min := int(^uint(0)) >> 1
	neighborgs := checkNSWE(ctx.Puzzle, m)
}

func checkNSWE(puzzle Tpuzzle, m map[string]interface{}) []s.SSuccessor {
	t.WhereToGo(puzzle)
}
